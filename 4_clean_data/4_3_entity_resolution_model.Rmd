...
deterministic_linking <- personal_info %>% 
  ...
  filter(
    # remove entities linking to itselves
    pid.x != pid.y,
...
# write_parquet(deterministic_linking, "./deterministic_linking_pairs.parquet")
```
...
# subset_blocks <- valid_blocks[1:10000,]
# 
...
# save subset_data
# write_parquet(subset_data, "subset_data.parquet")
```
...
```{r load data for testing record linkage}
# subset_data <- read_parquet("./subset_data.parquet", as_data_frame=FALSE)
# subset_data
```
...
Possible False Negative (undecided, still included in match)
"816070220170027",	"816070220170054"
"809133320140165", "809133320140189"
"221172520150083", "221172520150088"
"807095520180001","807095520180038"
"215013120190133","403316120190077"
"111110320200201","217110120200234"
...
updated_linked_pairs <- linked_pairs %>% 
  mutate(
    # create new columns with caregiver name's initials
    cg_initials.x = str_to_upper(str_replace_all(caregiver.x, '\\b(\\pL)\\pL{1,}|.', '\\1')),
    cg_initials.y = str_to_upper(str_replace_all(caregiver.y, '\\b(\\pL)\\pL{1,}|.', '\\1')),
    # if initials are the same --> default to TRUE, else default true_match to FALSE
    true_match = cg_initials.x == cg_initials.y,
  ...
  rows_update(false_pos, unmatched = "ignore") %>% 
  rows_update(false_neg, unmatched = "ignore") %>% 
  filter(true_match == TRUE) %>% 
  select(-true_match, -cg_initials.x, -cg_initials.y)
  
write_parquet(updated_linked_pairs, "./linked_data/post_inspection_linked_pairs.parquet")
```
